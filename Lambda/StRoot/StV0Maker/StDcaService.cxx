                       ...
                       const StThreeVectorF& pv, StThreeVectorF& xv0, StThreeVectorF& op1, StThreeVectorF& op2)
{
    ...
    if (d_p_pi >= (r_p + r_pi)
        ...
        // the very opposite direction (that means the pt of v0 is 0). FIXME: please notice the
        // hard cut 2.0cm above, you might change it for some purposes.
        fi_root1_p = 0.;
        fi_root2_p = 0.;
        if (d_p_pi >= (r_p + r_pi)) {
            fi_root1_pi = PI;
    ...
    if ((v0_root1[0] * v0_root1[0] + v0_root1[1] * v0_root1[1])
        < R2_TPC) { // XZHU: if the overlapping position is close (25 cm) to the outer layer of TPC. Even if the dca
                    // between two tracks is small, it is
        // not interesting to us anymore. We find the right periods here, which is not necessary for particles with pt >
        // 0.15GeV/c. in
        // recHyperon, the period finding is not used. ADDED: lots of daughters has small pt. we do not understand how
        // multiple periods
        // tracks are recorded (split to multiple tracks or not?). if we scan periods, we could have multiple signals
        // for a single lambda!!!
        ...
        n2 = proton->Flag; // XZHU: it is meanlingless to go forward in a helix to get the dca point where the particle
                           // (or helix) should orginates from!
        // so i suggest to decrease this flag to 0, since fi_root1_pi is [-2pi,2pi]. ADDED: it is not so simple due to
        // the TPC track
        // momentum resolution. we should keep the code as it is.
                       ...
                       double magnet, StThreeVectorF& xxi, StThreeVectorF& op)
{
    ...
    // lambda line ( in 2D ), it is still on
    // the lambda line
    ...
    if (dmax > r) { // no 2d crossing points but not too far either, then the newx and newy are the coordinates of the
                    // 2D closest point to the Lambda
        // line but still on the circle. //XZHU: should be rare, since it needs the momenta of lambda and K is in line
        // with each other.
        ...
        pyK = (xc - newx) / r * sign * sqrt(pxK * pxK + pyK * pyK);
             ...
             // meaning. newx, new y has the same
        // meaning in case 1
        ...
        float newz1 = z + sign0 * 2.0 * r * asin(da1 / 2.0) * pzK
                          / sqrt(pxK * pxK + pyK * pyK); // XZHU: only work in one period. for sign0=1, it is
        // weird to check the orign point in the forward
        // direction
        ...
        // straight line at the new position
