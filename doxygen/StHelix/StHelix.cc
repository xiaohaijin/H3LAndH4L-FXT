...
StHelix::StHelix(double c, double d, double phase, const StThreeVector<double>& o,
                 int h) {
...
/*!
 * \brief Returns the path length at the distance of closest
...
pair<double, double> StHelix::pathLength(double r) const {
  ...
  if (mSingularity) {
    ...
    double t20 = -mCosDipAngle * mCosDipAngle *
                 (2.0 * mOrigin.x() * mSinPhase * mOrigin.y() * mCosPhase + t12 -
                  t12 * t13 - t15 + t13 * t16);
    ...
    double t45 =
        2.0 * t5 - t35 + t21 + 2.0 - 2.0 * t1 * t2 - 2.0 * t43 - 2.0 * t43 * t5 + t8 * t3;
...
double StHelix::pathLength(const StThreeVector<double>& r,
  ...
  if (mSingularity) {
    double t = n.z() * mSinDipAngle + n.y() * mCosDipAngle * mCosPhase -
...
pair<double, double> StHelix::pathLengths(const StHelix& h, double minStepSize,
                                          double minRange) const {
  //
  //	Cannot handle case where one is a helix
  //  and the other one is a straight line.
  //
  if (mSingularity != h.mSingularity) return pair<double, double>(NoSolution, NoSolution);
  ...
  if (mSingularity) {
    ...
    StThreeVector<double> a(-mCosDipAngle * mSinPhase, mCosDipAngle * mCosPhase,
                            mSinDipAngle);
    StThreeVector<double> b(-h.mCosDipAngle * h.mSinPhase, h.mCosDipAngle * h.mCosPhase,
                            h.mSinDipAngle);
    double ab = a * b;
    double g = dv * a;
    double k = dv * b;
...
int operator==(const StHelix& a, const StHelix& b) {
  ...
  //
  return (a.origin() == b.origin() && a.dipAngle() == b.dipAngle() &&
          a.curvature() == b.curvature() && a.phase() == b.phase() && a.h() == b.h());
}
