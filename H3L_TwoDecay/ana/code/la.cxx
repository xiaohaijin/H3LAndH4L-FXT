...
int main(int n, char** argv)
{
    ...
    if (!t) {
        cout << "ERROR: no files are added to the chain!" << endl;
        return 0;
    }
    // bound them together
    v0dst dst(t);
    ...
    for (Long64_t jentry = 0; jentry < nentries; jentry++) {
        ...
        if (fabs(VertexZPos) >= 50.)
            continue; // VertexZPos=30->BufferPointer 100,should be 0-99

        for (Int_t i = 0; i < dst.nv0; i++) {

            if (dst.dau1nhitsfit[i] < 22)
                continue;
            if (dst.dau2nhitsfit[i] < 22)
                continue;

            if (dst.dau1nhitsdedx[i] < 15)
                continue;
            if (dst.dau2nhitsdedx[i] < 15)
                continue;

            if (fabs(dst.dau1nsigma[i]) > 3.0)
                continue;
            if (fabs(dst.dau2nsigma[i]) > 3.0)
                continue;

            if (dst.v0dca[i] > 1.0)
                continue;
            if (dst.dau1dca[i] < 0.0)
                continue;
            if (dst.dau2dca[i] < 2.0)
                continue;
            if (dst.dca1to2[i] > 0.7)
                continue;
            if (dst.v0declen[i] < 2.0)
                continue;
            if (dst.v0rdotp[i] < 0.0)
                continue;
...
double getCentBin(Int_t nrefmult)
{
    ...
    Int_t i;
    if (nrefmult < centbd[0] || nrefmult >= centbd[kCentBin])
        return -1;
    for (i = 0; i < kCentBin; i++) {
        if (centbd[i] <= nrefmult && centbd[i + 1] > nrefmult)
            break;
    }
